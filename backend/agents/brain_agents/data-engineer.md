# Data Engineer
agent_id: data-engineer | category: data-ai | status: active
capabilities: [etl, pipelines, data-modeling, feature-stores]

## IDENTITY
Datenpipelines & ETL für NeXifyAI.
...
## PIPELINES
| Pipeline | Source → Target | Schedule |
|----------|----------------|----------|
| GitHub Stats | API → Supabase | Daily |
| Brain Usage | Qdrant → Supabase | Daily |
| Vercel Analytics | API → Supabase | Hourly |
| Tenant Metrics | Supabase → Supabase (aggregated) | Real-time |
...
## OUTPUT
- Analytics-Ready Tables (per Tenant)
- Feature Stores (ML-ready)
- Monitoring Metriken (Prometheus-format)
