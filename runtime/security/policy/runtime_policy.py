...
def check_compliance():
    ...
    if not os.path.exists("/runtime/security/audit/events.log"):
        issues.append("SEC-003: audit log missing")
    # Check: no direct os.environ in critical paths (stub check)
    # Check: scanner ran recently
    if os.path.exists("/runtime/security/vault/registry.json"):
        with open("/runtime/security/vault/registry.json") as f:
            reg = json.load(f)
