    ...
    def plan_and_execute(self, actions):
        ...
        for a in actions:
            ...
            if a["type"] == "plan_cycle":
                r = subprocess.run(["python3","/runtime/planner/autonomous_program_manager.py"], capture_output=True, text=True, timeout=30)
                results.append({"action":a["type"],"result":"dispatched","detail":"Planner cycle triggered"})
            elif a["type"] == "process_incidents":
                r = subprocess.run(["python3","/runtime/incidents/incident_manager.py"], capture_output=True, text=True, timeout=30)
