      ...
      <div style={styles.container}>
        ...
        <div style={styles.header} data-testid="contract-view">
          <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
            <div style={{width:40,height:40,borderRadius:8,background:'rgba(254,155,123,0.1)',display:'flex',alignItems:'center',justifyContent:'center'}}><I n="gavel" style={{color:'#FE9B7B',fontSize:20}} /></div>
            <div>
        ...
        <div style={styles.metaGrid}>
          ...
          <div style={styles.metaCard}><div style={styles.metaLabel}>Gesamt</div><div style={{...styles.metaValue,color:'#FE9B7B',fontSize:'1rem'}}>{fmtCurrency(calc.gross_total)}</div></div>
        </div>
          ...
          <div style={styles.section}>
            ...
            {c.appendices_detail.map((a, i) => (
              <div key={a.appendix_id || i} style={styles.appendixCard}>
                ...
                {a.content?.value && <div style={{marginTop:4,fontSize:'.8125rem',color:'#FE9B7B',fontWeight:600}}>{fmtCurrency(a.content.value)}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        <div style={styles.section}>
          ...
          {(c.legal_module_definitions || []).map(lm => (
            <label key={lm.key} style={styles.legalRow} data-testid={`legal-${lm.key}`}>
              <input type="checkbox" checked={!!legalAccepted[lm.key]} onChange={e => setLegalAccepted({...legalAccepted, [lm.key]: e.target.checked})} style={{accentColor:'#FE9B7B',width:18,height:18}} />
              <div>
...
const styles = {
  ...
  tabBtnActive: { background:'rgba(254,155,123,0.1)', borderColor:'rgba(254,155,123,0.3)', color:'#FE9B7B' },
