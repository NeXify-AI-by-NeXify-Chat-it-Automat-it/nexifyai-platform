...
|---------|------|-------------|------|-----------|
| OpenRouter | 127.0.0.1:8420 (Brain API) | `/docker/OpenRouter-5afd/docker-compose.yml` | JWT | openrouter.ai |
...
docker compose up -d
```
Verify: `curl http://localhost:8420 (Brain API)/api/health` → `{"ok":true}`
...
curl -s http://localhost:8420 (Brain API)/api/health
...
|---------|-------------|-----|
| OpenRouter `/api/providers` → 401 | JWT not configured | Set `JWT_SECRET` in credentials.env, restart container |
